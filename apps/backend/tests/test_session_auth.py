...
from app.auth import AuthContext, get_optional_auth, validate_token
...
def app_instance(tmp_path):
    ...
    _dev_jwks = (
        "file:///workspaces/docroot/deploy/dev/jwks.json"
    )
    _settings = Settings(
    ...
    application.dependency_overrides[get_storage] = (
        lambda: storage
    )
...
def client(app_instance):
    ...
    return TestClient(
        app_instance, raise_server_exceptions=False
    )
...
def test_create_session_invalid_token_returns_401(
    client, app_instance
):
...
def test_create_session_valid_token_sets_cookie(
    client, app_instance
):
    ...
    try:
        resp = client.post(
            "/api/auth/session",
            headers={
                "Authorization": "Bearer valid.jwt.here"
            },
        )
    finally:
...
def test_create_session_no_auth_configured_returns_501(
    client, app_instance
):
    ...
    app_instance.dependency_overrides[get_settings] = (
...
def test_get_optional_auth_reads_session_cookie(
    app_instance
):
...
def test_get_optional_auth_bearer_takes_priority_over_cookie(
    app_instance
):
