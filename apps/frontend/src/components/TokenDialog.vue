<template>
  <v-btn prepend-icon="mdi-login" @click="onButtonClick">
  ...
  <v-dialog v-model="dialog" max-width="420">
    <v-card :title="t('authToken')">
      <v-card-text>
        ...
        <v-divider v-if="oidcAvailable" class="mb-4" />
        <v-text-field v-model="inputToken" autofocus :label="t('authToken')" type="password" />
      </v-card-text>
      <v-card-actions>
        <v-btn v-if="isAuthenticated()" color="error" @click="onClear">
          {{ t('logout') }}
        </v-btn>
        <v-spacer />
        ...
        <v-btn color="primary" :disabled="!inputToken" @click="onSet">
          {{ t('login') }}
        </v-btn>
      </v-card-actions>
    </v-card>
...
import { getUserManager, isAuthenticated, loginWithOidc, setToken } from '@/auth'
