<template>
  <v-dialog max-width="500" :model-value="modelValue">
    <v-card :title="t('uploadTitle')">
      <v-card-text>
        <v-file-input v-model="file" accept=".zip" :label="t('upload')" variant="outlined" />
        <v-text-field v-model="versionName" class="mt-2" :label="t('version')" variant="outlined" />
        <v-text-field
          v-model="localeName"
          class="mt-2"
        ...
        <v-alert v-if="successMsg" class="mt-2" type="success">
          {{ successMsg }}
        </v-alert>
      </v-card-text>
      <v-card-actions>
...
async function onSubmit() {
  ...
  try {
    ...
    errorMsg.value = t('uploadError') + ': ' + (error as Error).message
  } finally {
    loading.value = false
  }
}
