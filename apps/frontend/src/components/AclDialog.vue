        ...
        <template v-else-if="!loading">
          ...
          <v-table v-else density="compact">
            <thead>
              <tr>
                <th>{{ t('aclRoleName') }}</th>
                <th style="text-align: center">{{ t('aclCanRead') }}</th>
                <th style="text-align: center">{{ t('aclCanWrite') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr
...
<script setup lang="ts">
  ...
  async function load () {
    ...
    try {
      ...
      const aclMap = new Map<string, AclRole>(
        acl.roles.map(r => [r.role.toLowerCase(), r]),
      )
      ...
      rows.value = userRoles.map(lowerRole => {
        ...
        const roleName = existing?.role
          ?? me.roles.find(r => r.toLowerCase() === lowerRole)
          ?? lowerRole
        return {
          role: roleName,
  ...
  async function save (row: Row) {
    ...
    try {
      if (!row.read && !row.write) {
        await api.removeAclRole(
          props.namespace,
          row.role,
          token.value,
        )
      } else {
        await api.upsertAclRole(
          props.namespace,
          row.role,
          row.read,
          row.write,
